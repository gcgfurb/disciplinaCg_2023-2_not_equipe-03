...
namespace gcgcg
{
  internal class Objeto  // TODO: deveria ser uma class abstract ..??
  {
    ...
    public void setPonto(List<Ponto4D> a){
      this.pontosLista = a;
    }
    ...
    public void MatrizTranslacaoXYZ(double tx, double ty, double tz)
    {
      ...
      this.matriz = matrizTranslate.MultiplicarMatriz(matriz);
      ObjetoAtualizar();
    }
    ...
    public int menorDistancia(Ponto4D sruPonto){
      ...
      for(int i=0; i < this.pontosLista.Count; i++){
        ...
        conta = Matematica.distancia(sruPonto, pontosLista[i]);
        if(conta < max){
          max = conta;
          menorPonto = i;
        }
